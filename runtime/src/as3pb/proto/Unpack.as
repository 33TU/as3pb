package as3pb.proto
{
    ...
    public final class Unpack
    {
        ...
        /** Copy the requested range without resizing or changing the input's endian. */
        [Inline]
        public static function begin(context:UnpackContext, input:ByteArray, limit:uint):void
        {
            const start:uint = input.position;
            const end:uint = limit ? limit : input.length;

            if (end < start || end > input.length)
                throw new Error("Invalid protobuf message limit");

            const length:uint = end - start;
            const memory:ByteArray = context.memory;

            // AIR's unrolled intrinsic readers can range-check beyond an early terminator.
            // Keep physical headroom while preserving the logical message limit below.
            const required:uint = Math.max(ApplicationDomain.MIN_DOMAIN_MEMORY_LENGTH, Number(length) + 10);
            ...
            {
                memory.position = 0;
                memory.writeBytes(input, start, length);
            }
        ...
        /** Publish the consumed cursor and restore the caller's memory binding, including on failure. */
        [Inline]
        public static function end(context:UnpackContext):void
        {
            DOMAIN.domainMemory = context.previous;
            context.bytes.position = context.start + context.position;
        ...
        {
            out.low = readFixed32(src);
            out.high = readFixed32(src);
        }
        ...
        public static function readString(src:UnpackContext):String
        {
            ...
            bytes.position = src.start + src.position;
        ...
        public static function readBytesInto(src:UnpackContext, dest:ByteArray):void
        {
            ...
            if (length)
            {
                if (src.position > src.limit || length > src.limit - src.position)
                    throw new EOFError("Truncated protobuf input");
                src.bytes.position = src.start + src.position;
        ...
        public static function captureUnknownField(src:UnpackContext, tag:uint, out:ByteArray):void
        {
            ...
            out.writeBytes(src.bytes, src.start + start, src.position - start);
        }
