...
package example.game
{
    ...
    public final class Move
    {
        ...
        public static function deserializeMemory(src:UnpackContext, dst:example.game.Move = null, limit:uint = 0, reset:Boolean = true):example.game.Move
        {
            if (!dst)
                ...
                while (src.position < end)
                {
                    ...
                    switch (tag)
                    {
                        case 10:
                        {
                            ...
                            dst.from = example.game.Point.deserializeMemory(src, dst.from, src.position + messageLength, false);
                            break;
                        }
                        case 18:
                        {
                            ...
                            if (src.position > src.limit || messageLength > src.limit - src.position)
                                throw new EOFError("Truncated protobuf input");
                            dst.to = example.game.Point.deserializeMemory(src, dst.to, src.position + messageLength, false);
                            break;
                        }
                        case 24:
                        {
                            Unpack.readSint64(src, dst.tick);
                            break;
                        }
                        default:
                        {
